...
class CBM(nn.Module):
    ...
    def calculate_summaries(self, patient_batch, epsilon_denom=1e-8):
        ...
        x_mean = torch.sum(batch_measurement_ind * batch_changing_vars, dim=1, keepdim=True) / (torch.sum(batch_measurement_ind, dim=1, keepdim=True) + epsilon_denom)
        ...
        # print("batch_measurement_ind", batch_measurement_ind.shape, (batch_measurement_ind is None or torch.isnan(batch_measurement_ind).any()))
