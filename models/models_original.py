...
class CBM(BaseCBM):
    ...
    def get_model_path(self, base_path, dataset, seed=None, pruning="", ending=".pt"):
        if seed is None:
            name = f"num_concepts_{self.num_concepts}_use_indicators_{self.use_indicators}_use_only_last_timestep_{self.use_only_last_timestep}{ending}"
        else:
            name = f"num_concepts_{self.num_concepts}_use_indicators_{self.use_indicators}_use_only_last_timestep_{self.use_only_last_timestep}_seed_{seed}{ending}"
