...
class Masking(object):
        ...
        if mode == 'constant':
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    self.masks[name][:] = (torch.rand(weight.shape) < density).float().data.cuda(weight.device)
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    if name in self.name_to_32bit:
                        print('W2')
                    self.masks[name][:] = (weight != 0.0).float().data.cuda(weight.device)
                    self.baseline_nonzero += weight.numel()*density
            self.apply_mask()
        elif mode == 'linear':
            ...
            for name, weight in module.named_parameters():
                ...
                self.masks[name][:] = (torch.rand(weight.shape) < prob).float().data.cuda(weight.device)
            self.apply_mask()
    ...
    def add_module(self, module, density, sparse_init='constant', remove_param_names=()):
        ...
        for name, tensor in module.named_parameters():
            ...
            self.masks[name] = torch.zeros_like(tensor, dtype=torch.float32, requires_grad=False).cuda(tensor.device)
